...
init()
{
	...
	game["strings"]["airdrop_hint"] = "^7Press and hold ^3[{+activate}] ^7for a Care Package"; // Custom message (:
	...
	game["strings"]["nuke_hint"] = &"MP_NUKE_PICKUP"; // Erm?
	...
	if ( getdvar( "g_gametype" ) == "sd" ) {
		addCrateType( "airdrop",	"ammo", 			getDvarInt( "scr_airdrop_ammo", 10000 ),		::ammoCrateThink );
	} else {
...
dropTimeOut( dropCrate, owner )
{
	...
	if ( dropCrate.dropType == "nuke_drop" ) // Uhhh, not needed? Leaving for save messures.
		return;	
	
	if ( getdvar( "g_gametype" ) == "sd" ) {
		maps\mp\gametypes\_hostmigration::waitLongDurationWithHostMigrationPause( 3600.0 );
	} else {
...
/* THIS IS INCLUDED BECAUSE I PULLED IT FROM MOSSY <333
...
airdropCrateThink( dropType )
{
	self endon ( "death" );

	crateSetupForUse( game["strings"]["airdrop_hint"], "all", maps\mp\killstreaks\_killstreaks::getKillstreakCrateIcon( self.crateType ) );
...
{
	if ( getdvar( "g_gametype" ) == "sd" ) {
		self maps\mp\killstreaks\_killstreaks::giveKillstreak( "airdrop", true ); // Less suss.
	}
