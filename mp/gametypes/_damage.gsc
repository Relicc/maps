...
Callback_PlayerDamage_internal( eInflictor, eAttacker, victim, iDamage, iDFlags, sMeansOfDeath, sWeapon, vPoint, vDir, sHitLoc, psOffsetTime )
{
	...
	// Found somewhere on the forums, can't remember where.

	if (isSubStr(sWeapon, "cheytac_") || isSubStr(sWeapon, "barrett_") || isSubStr(sWeapon, "concussion_grenade_mp") || isSubStr(sWeapon, "throwingknife_mp")) {
		switch( sHitLoc ) {
			case "helmet":
			case "head":
			case "neck":
		...
		if (!isSubStr(sWeapon, "throwingknife_mp") && sMeansOfDeath == "MOD_IMPACT") { // Other means of impact, stun, flash, frag?
			iDamage = 1;
		}
	} else {
