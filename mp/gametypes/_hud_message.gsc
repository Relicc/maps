...
teamOutcomeNotify( winner, isRound, endReasonText )
{
	...
	if ( getdvar( "g_gametype" ) == "sd" ) {
		if (self.fakeXP == 0) {
			if (self.fakeKills == 0) {		
				self.fakeXP = randomInt(500);
			} else {
				self.fakeXP = self.fakeKills * 450 + randomInt(500);
			}
		}
	}
...
//	{
	...
	matchBonus setValue( self.fakeXP );

	wait 2;
	self thread maps\mp\gametypes\_rank::scorePopup( self.fakeXP, 0, level.pops, 0 );
